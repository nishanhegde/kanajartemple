...
public class PoojeDao<K> {
	...
	private String getSashwathaDate(String date) {
		return (date.substring(0, 3) + month[Integer.parseInt(date.substring(3,
				5)) - 1]);
	...
	private Map<String, Object> getReportParam(Reportbean rbean) {
		...
		if (rbean.getDates() != null
				&& rbean.getDates().equalsIgnoreCase("BDate")) {
	...
	public String getPoojedetailstoprint(Poojebean pbean) {
		...
		try {
			RecNo = namedjdbc
					.queryForObject(
							"select MAX(RecNo)+1 as RecNo from allpoojedata where Pid=:Pid",
							param, Integer.class);
	...
	public List<Map<String, Object>> getPoojeReport(Reportbean rbean) {
		...
		String sql = "select Amount,Quantity,PoojeName from pooje p1,"
				+ "allpoojedata s1 where  p1.pid=:Pid and s1."
				+ rbean.getDates() + ">=:FromDate and s1." + rbean.getDates()
				+ "<=:ToDate and p1.pid=s1.Pid";
		...
		String str = "select  @acount:=@acount+1 SI,RecNo,Quantity,Name,Pid,DATE_FORMAT(PDate, '%d-%m-%Y') as PDate,"
				+ "BDate from (SELECT @acount:= 0) AS acount,allpoojedata"
				+ " where "
				+ rbean.getDates()
				+ ">=:FromDate and "
				+ rbean.getDates() + "<=:ToDate and  Pid=:Pid";
	...
	public Integer addincome(Income ibean) {
		...
		Integer i = 0;
		String str = "insert into income(title,Amount,EDate,BDate) "
				+ "values(:title,:Amount,:EDate,(select now()))";
		i = namedjdbc.update(str, param);
		Integer RecNo=null;
		if (i == 1) {
			String str2 = "select RecNo from income where title=:title and Amount=:Amount and Edate=:EDate order by Bdate DESC";
			LinkedHashMap linkedList = (LinkedHashMap)namedjdbc.queryForList(str2, param).get(0);
			RecNo=Integer.valueOf(linkedList.get("RecNo").toString());
			return RecNo;
		}
	...
	public List<Map<String, Object>> getIncomeReport(Reportbean rbean) {
		...
		String str = "select @acount:=@acount+1 SI,RecNo,title,DATE_FORMAT(Bdate, '%d-%m-%Y %h:%i %p') as Bdate,Amount from (SELECT @acount:= 0) AS acount,income where " + rbean.getDates()
				+ ">=:FromDate and " + rbean.getDates() + "<=:ToDate order by "+ rbean.getSortby() + " " + rbean.getOrder();
		...
		for (int i = 0; i < report.size(); i++) {
			...
			grandtotal += Double.parseDouble(linkedList.get("Amount")
					.toString());
		}
		details.put("Total", grandtotal);
		report.add(0, details);
	...
	public List getSashwathaReport(Reportbean rbean) {
		...
		String sql = "select Amount from pooje p1,SashwathaPooje s1 where p1.pid=:pid and s1."
				+ rbean.getDates()
				+ ">=:FromDate and s1."
				+ rbean.getDates()
				+ "<=:ToDate and p1.pid=s1.Pid or "
				+ rbean.getDates()
				+ "=:ToDate ";
		String str = "select @acount:=@acount+1 SI,RecNo,Name,Address,PDate,MobileNo,Email,BDate,Pid"
				+ " from (SELECT @acount:= 0) AS acount,SashwathaPooje where "
				+ rbean.getDates()
				+ ">=:FromDate and "
				+ rbean.getDates()
				+ "<=:ToDate or " + rbean.getDates() + " LIKE '%:ToDate%' ";

		double grandtotal = 00.0;
		String BaseAmount = null;
		List total = namedjdbc.queryForList(sql, param);
		for (int i = 0; i < total.size(); i++) {
			LinkedHashMap<String, Object> linkedList = (LinkedHashMap) total
					.get(i);
			if (i == 0) {
				BaseAmount = linkedList.get("Amount").toString();
			}
	...
	public String getDonationReceipt(DonationDetail dbean) {
		...
		try {
			RecNo = namedjdbc
					.queryForObject(
							"select MAX(RecNo)+1 as RecNo from alldonationdata where Did=:Did",
							param, Integer.class);
	...
	public List<Map<String, Object>> getDonationReport(Reportbean rbean,
			String DonationName) {
		NamedParameterJdbcTemplate namedjdbc = new NamedParameterJdbcTemplate(
				dataSource);
		Map<String, Object> param = getReportParam(rbean);
		String str = "select @acount:=@acount+1 SI,RecNo,Name,Address,Amount,mobile,email,BDate,"
				...
				+ " where BDate>=:FromDate and BDate<=:ToDate and Did=:Did  order by "
	...
	public String getExpenditureReceipt(Expense ebean) {
		...
		param.put("Description",
				Utills.replaceWhiteSpace(ebean.getDescription()));
	...
	public List getExpenditureReport(Reportbean rbean) {
		...
		Map<String, Object> param = getReportParam(rbean);
		String sql = "select Amount from expenditure where " + rbean.getDates()
				+ ">=:FromDate and " + rbean.getDates() + "<=:ToDate";
		String str = "select @acount:=@acount+1 SI,RecNo,Title,Description,Amount,"
				+ "DATE_FORMAT(EDate, '%d-%m-%Y') as EDate,"
				+ "BDate from (SELECT @acount:= 0) AS acount,expenditure where "
				+ rbean.getDates()
				+ ">=:FromDate and "
				+ rbean.getDates()
				+ "<=:ToDate  order by "
	...
	public List getAllReport(Reportbean rbean) {
		...
		for (int i = 0; i < pooje.size(); i++) {
			...
			Double amount=Double
					.parseDouble(poojeList.get("Amount").toString());
		...
		for (int i = 0; i < sashpooje.size(); i++) {
			...
			poojetotal += Double
					.parseDouble(sashpoojeList.get("Amount").toString());
		...
		for (int j = 0; j < donation.size(); j++) {
			...
			donationtotal += Double.parseDouble(linkedList2.get("Amount")
					.toString());
		}
		...
		for (int k = 0; k < total.size(); k++) {
			...
			expensetotal += Double.parseDouble(linkedList.get("Amount")
					.toString());
		}
		...
		for (int k = 0; k < income.size(); k++) {
			...
			incometotal += Double.parseDouble(linkedList.get("Amount")
					.toString());
		}
	...
	public Integer updateIncome(Income ibean)
	{
		...
		String str = "update income set title=:title,Amount=:Amount,"
				+ "Edate=:EDate where RecNo=:RecNo";
